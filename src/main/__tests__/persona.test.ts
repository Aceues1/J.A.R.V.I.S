...
describe('persona system prompt', () => {
  ...
  it('grants the live web-search capability with strict honesty rules', () => {
    ...
    expect(SYSTEM_PROMPT).toMatch(/lead with the concrete headlines and facts/i)
    expect(SYSTEM_PROMPT).toMatch(/names, products, numbers, dates/i)
    expect(SYSTEM_PROMPT).toMatch(/name the outlet when it adds weight/i)
    expect(SYSTEM_PROMPT).toMatch(/Generic summaries.*are a failed answer/is)
